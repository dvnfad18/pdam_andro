...
class PeminjamanPage extends StatefulWidget {
  ...
  PeminjamanPage({required this.idCust});
  @override
  _SewaAsetPageState createState() => _SewaAsetPageState();
}

class _SewaAsetPageState extends State<PeminjamanPage> {
  List<dynamic> form = [];
  ...
  Future<void> getUserData() async {
    try {
      ...
      final idCust = prefs.getInt('idCust');
      final namaCust = prefs.getString('namaCust');
      final email_Cust = prefs.getString('email_Cust');

      setState(() {
        this.namaCust = namaCust ?? '';
        _namaController.text = namaCust ?? '';
      });

      print('User Name: $namaCust');
    } catch (e) {
      print('$e');
  ...
  void getData() async {
    try {
      final lapanganData = await LapanganService.show(widget.idCust);
      setState(() {
        form = lapanganData;
        _lapanganController.text = form[0]['namaAset'].toString();
