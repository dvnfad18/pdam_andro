...
import '../Akun/AkunMenu.dart';
import '../Akun/AkunScreen.dart';
...
class _AkunPicState extends State<AkunPic> {
  ...
  Future<void> _handlephoto() async {
    ...
    SessionManager.getIdCust().then((idCust) async {
      ...
      request.fields['idCust'] = idCustString;
      request.files
  ...
  Future<Users> getData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('ko ga muncul ya');
      }
    } catch (e) {
      print(e.toString());
      throw Exception('Failed to load data');
