...
class DetilAset extends StatefulWidget {
  GetDetilAset notaData;
  DetilAset({Key? key, required this.notaData}) : super(key: key);

  @override
  _DetilAsetState createState() => _DetilAsetState();
}

class _DetilAsetState extends State<DetilAset> {
  late GetDetilAset listNota;
  ...
  Future<void> fetchData(int index) async {
    try {
      String idAset = widget.notaData.idAset.toString();
      GetDetilAset data = await ServiceApiDetilAset(idAset).getData();
      setState(() {
        // listNota = data;
  ...
  Widget favoriteIcon(index) {
    int? productId = widget.notaData.idAset;
        ...
        Positioned(
          ...
          child: Row(
            children: [
              ...
              Text(
                widget.notaData.nama_aset.toString(),
                style: TextStyle(
                  fontSize: 22,
  ...
  Widget image(GetDetilAset product) {
    if (product.gambar != null) {
      String imageUrl =
          "https://c5de-202-80-213-12.ngrok-free.app/" + widget.notaData.gambar.toString();
      return Container(
        height: 128,
        width: 128,
        color: Colors.grey,
  ...
  Widget text(GetDetilAset product) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              if (product.nama_aset != null && product.nama_aset!.isNotEmpty)
                Text(
                  product.nama_aset.toString(),
        ...
        Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  if (widget.notaData.nama_aset != null && widget.notaData.nama_aset!.isNotEmpty)
                    Text(
                      widget.notaData.nama_aset.toString(),
                      style: const TextStyle(
                        fontSize: 14,
              ...
              Row(children: [
                ...
                Text(
                  ...
                  "Stok : "+     widget.notaData.jumlahAset.toString(),
                  style: const TextStyle(
                    fontSize: 14,
                    fontWeight: FontWeight.bold,
                  ),
  ...
  Future<void> _handleAdddatafavorit(BuildContext context, int index) async {
    ...
    SessionManager.getIdCust().then((idCust) {
      ...
      http.post(Uri.parse(ApiConnect.add_datafavorit), body: {
        "idCust": idCustString,
        "idAset": widget.notaData.idAset.toString(),
      }).then((response) {
